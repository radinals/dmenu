...
drawmenu(void)
{
	...
	if (show_prompt) {
		...
		if (lines > 0 && prompt && *prompt) {
			drw_setscheme(drw, scheme[SchemeSel]);
			drw_text(drw, 0, 0, mw, bh * prompt_input_separator_factor,
                    (mw / 2) - promptw, prompt, 0);
		}
	}
...
keypress(XKeyEvent *ev)
{
	...
	if (ev->state & ControlMask) {
		...
		case XK_Y:
			XConvertSelection(
			    dpy, (ev->state & ShiftMask) ? clip : XA_PRIMARY,
...
readstdin(void)
{
	...
	for (i = 0; (len = getline(&line, &linesiz, stdin)) != -1; i++) {
		...
		if (i + 1 >= itemsiz / sizeof *items) {
			itemsiz += 256;
			if (!(items = realloc(items, (itemsiz += BUFSIZ))))
...
setup(void)
{
	...
	if (parentwin == root && (info = XineramaQueryScreens(dpy, &n))) {
		...
		*/
		...
		if (centered) {
			mw = MIN(MAX(max_textw() + promptw,
			             info[i].width / menu_width_ratio),
			         info[i].width);
			x = info[i].x_org + ((info[i].width - mw) / 2);
			y = info[i].y_org +
			    ((info[i].height - mh) / menu_height_ratio);
		} else {
			x = info[i].x_org;
			y = info[i].y_org + (topbar ? 0 : info[i].height - mh);
			mw = info[i].width;
	...
	win =
	    XCreateWindow(dpy, root, x, y, mw, mh, border_width, CopyFromParent,
